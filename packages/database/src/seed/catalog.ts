...
export async function seedCatalog(prisma: PrismaClient): Promise<void> {
  ...
  for (const child of CHILD_CATEGORIES) {
    ...
    await prisma.category.upsert({
      ...
      create: { name: child.name, slug: child.slug, position: child.position, parentId: parent?.id },
  ...
  for (const spec of PRODUCTS) {
    ...
    const discountPercent = Math.round(
      (1 - spec.outletPriceMinor / spec.originalPriceMinor) * 100,
    );
  ...
  console.log(`Seeded ${BRANDS.length} brands, ${CATEGORIES.length + CHILD_CATEGORIES.length} categories, ${PRODUCTS.length} products`);
}
