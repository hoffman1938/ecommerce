        ...
        <div className="space-y-8 lg:col-span-2">
          ...
          <section className="rounded-lg border border-gray-200 bg-white p-5">
            ...
            {Object.keys(form.formState.errors.shippingAddress ?? {}).length > 0 ? (
              <p className="mt-2 text-xs text-red-600">Please complete the highlighted address fields.</p>
            ) : null}
          ...
          <section className="rounded-lg border border-gray-200 bg-white p-5">
            ...
            <div className="space-y-2">
              {quote.shippingMethods.map((method) => (
                <label
                  key={method.id}
                  className="flex cursor-pointer items-center justify-between rounded-md border border-gray-200 px-4 py-3 text-sm has-[:checked]:border-gray-900"
                >
                  <span className="flex items-center gap-3">
                    <input type="radio" value={method.id} {...form.register('shippingMethod')} />
                    <span>
                      <span className="font-medium">{method.label}</span>
                      <span className="block text-xs text-gray-500">{method.estimatedDays}</span>
                    </span>
                  </span>
                  <span>
            ...
            <label className="mt-4 block text-sm">
              <span className="mb-1 block font-medium text-gray-700">Order note (optional)</span>
              <textarea
