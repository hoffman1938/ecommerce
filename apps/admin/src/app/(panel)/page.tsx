  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <StatCard label="Active reservations" value={String(stats.activeReservationCount)} href="/reservations" />
        ...
        <StatCard label="Active campaigns" value={String(stats.activeCampaignCount)} href="/campaigns" />
        <StatCard label="Upcoming campaigns" value={String(stats.upcomingCampaignCount)} href="/campaigns" />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <section className="rounded-lg border border-gray-200 bg-white p-4">
          ...
          <table className="admin-table">
            <thead>
              <tr><th>SKU</th><th>Product</th><th className="text-right">Available</th></tr>
            </thead>
            <tbody>
              {stats.lowStockVariants.map((row) => (
                <tr key={row.variantId}>
                  ...
                  <td className={`text-right font-semibold ${row.availableQuantity === 0 ? 'text-red-600' : 'text-amber-600'}`}>
                    {row.availableQuantity}
                  </td>
                </tr>
              ))}
              {stats.lowStockVariants.length === 0 ? (
                <tr><td colSpan={3} className="text-gray-400">All good — nothing running low.</td></tr>
              ) : null}
            </tbody>
          </table>
        </section>
        ...
        <section className="rounded-lg border border-gray-200 bg-white p-4">
          ...
          <table className="admin-table">
            <thead><tr><th>Day</th><th className="text-right">Orders</th><th className="text-right">Revenue</th></tr></thead>
            <tbody>
              ...
              {stats.salesByDay.length === 0 ? (
                <tr><td colSpan={3} className="text-gray-400">No sales yet.</td></tr>
              ) : null}
            </tbody>
          </table>
        </section>
        ...
        <section className="rounded-lg border border-gray-200 bg-white p-4">
          ...
          <table className="admin-table">
            <thead><tr><th>Brand</th><th className="text-right">Units</th><th className="text-right">Revenue</th></tr></thead>
            <tbody>
          ...
          {stats.salesByCampaign.length > 0 ? (
            <table className="admin-table mt-4">
              <thead><tr><th>Campaign</th><th className="text-right">Units</th><th className="text-right">Revenue</th></tr></thead>
              <tbody>
